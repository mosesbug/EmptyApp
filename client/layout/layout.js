...
//**********Speech Recognition Events*****************************
    ...
    if(jsonObject.result.metadata.intentName ==="join a course") {
      ...
      courseObject = Courses.find({className: courseName}).fetch()[0];
        ...
        Router.go("/coursePage/"+courseId);
        respond("Successfuly enrolled in " + courseName);
        toastr.info("Successfuly enrolled in " + courseName);
    ...
    // Going to your class
     ...
     var language = jsonObject.result.parameters.language;
     var classNumber = jsonObject.result.parameters.number;
     var className1 = language.charAt(0).toUpperCase() + language.slice(1) + " " + classNumber;
     ...
     for(i=0; i<arrayOfClasses.length; i++) {
       ...
       myclassName = courseOne.className;


        if(myclassName === className1) {
          console.log("Going to your class...");
          var path = courseOne._id;
          Router.go('/coursePage/' + path);
          toastr.info("Going to " + mycourseName)
          break;
        } else {
    ...
    else  if (jsonObject.result.metadata.intentName ==="go to your assignment") {
    ...
    var numOfAssignments = Assignments.find({'metadata.course':globalCourseId}).count();
    ...
    if(numOfAssignments>0) {
      ...
      console.dir(Assignments.find({'metadata.course':globalCourseId}).fetch());
      var assignmentArray = Assignments.find({'metadata.course':globalCourseId}).fetch();
